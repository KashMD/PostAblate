...
export function FollowUpOptionCard({ title, description, href, badge }: { title: string; description: string; href: string; badge?: string }) {
  return (
    <Link href={href} className="rounded-lg border border-slate-200/80 bg-white/95 p-6 shadow-card transition hover:-translate-y-0.5 hover:border-teal/30 hover:shadow-soft focus:outline-none focus:ring-4 focus:ring-teal/15">
      ...
      {badge ? <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-teal">{badge}</p> : null}

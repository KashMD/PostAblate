...
export default function OneMonthPage() {
  ...
  const result: TriageResult = useMemo(() => {
    ...
    return {
      ...evaluated,
      clinicianSummary: buildFollowUpSummary("PostAblate One-Month AF Ablation Summary", values, evaluated.category)
    };
  }, [values]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      <PageHeader
        eyebrow="One-month follow-up"
        title="One-month AF ablation recovery and outcome follow-up"
        description="Let's complete your 1-month AF ablation recovery check-in. If you are having severe or urgent symptoms, please seek emergency care or contact your EP team now."
      />
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_0.9fr]">
        <form className="grid gap-5" onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }}>
          ...
          <button className="rounded-lg bg-navy px-5 py-3 text-sm font-semibold text-white shadow-sm" type="submit">Submit one-month follow-up</button>
        </form>
        <aside className="lg:sticky lg:top-28 lg:self-start">{submitted ? <TriageResultCard result={result} /> : <div className="rounded-lg border border-slate-200 bg-white p-5 text-sm leading-6 text-slate-600 shadow-soft">Your one-month safety category and clinician summary will appear here after submission.</div>}</aside>
      </div>
    </div>
  );
}
